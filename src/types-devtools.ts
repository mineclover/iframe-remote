 ...
 */
export function validateParamMeta(param: ParamMeta): string[] {
  const errors: string[] = []
  ...
  if (param.type === 'select' && 'options' in param) {
    if (!param.options || param.options.length === 0) {
      errors.push(`Parameter "${param.name}": select type requires options`)
    }
  }

  if (param.type === 'number' && 'min' in param && 'max' in param) {
    const numParam = param as NumberParamMeta
    if (
      numParam.min !== undefined &&
      numParam.max !== undefined &&
      numParam.min > numParam.max
    ) {
 ...
 */
export function validateFunctionMeta(meta: FunctionMeta): string[] {
  const errors: string[] = []

  if (meta.params) {
    meta.params.forEach((param, index) => {
      const paramErrors = validateParamMeta(param)
      errors.push(...paramErrors.map((e) => `Param[${index}]: ${e}`))
    })
  }
